  ...
  SubShader{
    ...
    Pass {
      ...
      #define apply_proc_tex4D() \  
        ...
        float4 f = (_GridFreq * (i.uv) + 0.5) + float4(_OffsetX, _OffsetY, _OffsetZ, _OffsetW); \
        f = (f - floor(f)); \
        float4 q = max(f - 1.0 + detail, 0.0) + max(W - f, 0.0); \
        q = min(q, min(W, detail)); \
        float grid = smoothstep(0, 1, 2* frac(q.x + q.y + q.z + q.w)) / detail; \
        color.rgb = color.rgb * 0.1 + lerp(color.rgb, _Color2.rgb, grid); 
        /*color.rgb = color.rgb * 0.1 + lerp(color.rgb, origColor.rgb, grid2); */
      #define apply_proc_tex5D() \
        ...
        detail = 1.0 - (1.0 - detail) * (1.0 - detail); \
        float4 f = _GridFreq * (i.uv + float4(_OffsetX, _OffsetY, _OffsetZ, _OffsetW)) + 0.5;  \
