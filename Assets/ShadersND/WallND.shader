...
//#endif
      ...
      #define apply_proc_tex4D() \
        ...
        i.uv.y += step(1.0, fmod(abs(i.uv.x * 2), 2.0)) + step(1.0, fmod(abs(i.uv.z * 2), 2.0)); \ 
        float4 f = _GridFreq * i.uv + 0.5;\
        f.xz *= 4; \
        f = f - floor(f); \
        float4 q = max(f - 1.0 + detail, 0.0) + max(float4(W, 0.025, W, W) - f, 0.0); \
        q = min(q, min(W, detail)); \
        float grid = max(max(q.x, q.y / 2), max(q.z, q.w)) / detail; \
